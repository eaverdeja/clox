...
static void runtimeError(const char* format, ...) {
    ...
    // We look into the topmost CallFrame on the stack using the current
    // framecount index minus one. That’s because the interpreter advances past
    // each instruction before executing it. So, at the point that we call
...
#define BINARY_OP(valueType, op)                          \
    ...
    for (;;) {
        ...
        dissasembleInstruction(&frame->function->chunk,
