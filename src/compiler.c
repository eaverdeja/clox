...
#ifdef DEBUG_PRINT_CODE
    if (!parser.hadError) {
        dissasembleChunk(currentChunk(), function->name != NULL
                                             ? function->name->chars
                                             : "<script>");
    }
#endif
...
static uint8_t identifierConstant(Token* name) {
    // Takes the given token and adds its lexeme to the chunk’s constant table
    // as a string. Returns the index of that constant in the constant table.
...
static void and_(bool canAssign) {
    ...
    // false, so we skip the right operand and leave the left-hand side value as
...
static void or_(bool canAssign) {
    ...
    // statement, which is an unconditional jump over the code for the
    // right operand.
...
static void varDeclaration() {
    ...
    if (match(TOKEN_EQUAL)) {
        ...
        // This desugars var declarations to:
        // var a = nil;
        emitByte(OP_NIL);
    }
...
static void forStatement() {
    ...
    if (!match(TOKEN_RIGHT_PAREN)) {
        ...
        expression();
        emitByte(OP_POP);  // Pop increment result.
        consume(TOKEN_RIGHT_PAREN, "Expect ')' after for clauses.");

        // This is the main loop that takes us back to the top of the for
        // loop—right before the condition expression
        emitLoop(loopStart);
        // We change loopStart to point to the offset where the increment
        ...
        // expression instead of the top of the loop like it does when there is
        // no increment.
